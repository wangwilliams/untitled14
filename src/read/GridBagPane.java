...
public class GridBagPane extends JPanel {
     ...
     * @param file the name of the XML file that describes
     * the pane's components and their position
     */
    public GridBagPane(File file){
        ...
        try{
            ...
            if(file.toString().contains("-schema")){
                ...
                final String W3C_XML_SCHEMA = "http://www.w3.grg/2001/Schema";
    ...
    private void parseGridbag(Element e){
        ...
        for(int i =0;i<rows.getLength();i++){
            ...
            for(int j=0;j<((NodeList) cells).getLength();j++){
    ...
    private Object parseValue(Element e){
        ...
        if (child.getTagName().equals("boolean")) return new Boolean(text);
        if (child.getTagName().equals("string")) return text;
        else return null;
    }
}
